...
int H2 = 256; // Hidden Layer 2 Size
int L = 10; // Output Size



/* define all the inputs and outputs */
int **X; int X_dimX = B; int X_dimY = D;
int **X_t; int X_t_dimX = D; int X_t_dimY = B;
int **Y1; int Y1_dimX = B; int Y1_dimY = H1;
int **Y1_t; int Y1_t_dimX = H1; int Y1_t_dimY = B;
int **Y2; int Y2_dimX = B; int Y2_dimY = H2;
int **W1; int W1_dimX = D; int W1_dimY = H1;
int **W2; int W2_dimX = H1; int W2_dimY = H2;
int **W2_t; int W2_t_dimX = H2; int W2_t_dimY = H1;
int **WR; int WR_dimX = H2; int WR_dimY = L;
...
int **WR_t; int WR_t_dimX = L; int WR_t_dimY = H2;
int **M2_1; int M2_1_dimX = B; int M2_1_dimY = H2;
int **M2_2; int M2_2_dimX = H1; int M2_2_dimY = H2;
int **MR1; int MR1_dimX = B; int MR1_dimY = L;
int **M1_2; int M1_2_dimX = D; int M1_2_dimY = H1;
...
void *controller(void *id) {
    ...
    multiply_func[MUL_FUNC](X, W1, Y1, X_dimX, X_dimY, W1_dimX, W1_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    if (thread_id == 0) { end = __rdtsc(); cout << "Y1     : " << end - start << endl; }
    
    if (thread_id == 0) start = __rdtsc();
    multiply_func[MUL_FUNC](Y1, W2, Y2, Y1_dimX, Y1_dimY, W2_dimX, W2_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    if (thread_id == 0) { end = __rdtsc(); cout << "Y2     : " << end - start << endl; }

    if (thread_id == 0) start = __rdtsc();
    multiply_func[MUL_FUNC](Y2, WR, R1, Y2_dimX, Y2_dimY, WR_dimX, WR_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    ...
    if (thread_id == 0) start = __rdtsc();
    elem_wise_multi_dim(R2, R2_1, dR2_dR1, R2_dimX, R2_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    ...
    if (thread_id == 0) start = __rdtsc();
    multiply_func[MUL_FUNC](MR1, WR_t, M2_1, MR1_dimX, MR1_dimY, WR_t_dimX, WR_t_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    if (thread_id == 0) { end = __rdtsc(); cout << "M2_1   : " << end - start << endl; }

    if (thread_id == 0) start = __rdtsc();
    multiply_func[MUL_FUNC](Y1_t, M2_1, M2_2, Y1_t_dimX, Y1_t_dimY, M2_1_dimX, M2_1_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    if (thread_id == 0) { end = __rdtsc(); cout << "M2_2   : " << end - start << endl; }

    if (thread_id == 0) start = __rdtsc();
    multiply_func[MUL_FUNC](M2_1, W2_t, M1_1, M2_1_dimX, M2_1_dimY, W2_t_dimX, W2_t_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
    if (thread_id == 0) { end = __rdtsc(); cout << "M1_1   : " << end - start << endl; }

    if (thread_id == 0) start = __rdtsc();
    multiply_func[MUL_FUNC](X_t, M1_1, M1_2, X_t_dimX, X_t_dimY, M1_1_dimX, M1_1_dimY, thread_id, NUM_THREADS);
    pthread_barrier_wait(&barrier);
...
{  
    ...
    /* Allocate memory for all the inputs and outputs */
    {
        ...
        Y2 = mem_alloc(Y2_dimX, Y2_dimY);
        W2 = mem_alloc(W2_dimX, W2_dimY);
        ...
        WR_t = mem_alloc(WR_t_dimX, WR_t_dimY);
        R2 = mem_alloc(R2_dimX, R2_dimY);
        R2_1 = mem_alloc(R2_1_dimX, R2_1_dimY);
        dR2_dR1 = mem_alloc(dR2_dR1_dimX, dR2_dR1_dimY);
        M1_1 = mem_alloc(M1_1_dimX, M1_1_dimY);
        M1_2 = mem_alloc(M1_2_dimX, M1_2_dimY);
        M2_1 = mem_alloc(M2_1_dimX, M2_1_dimY);
        M2_2 = mem_alloc(M2_2_dimX, M2_2_dimY);
